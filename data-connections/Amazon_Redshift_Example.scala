...
val tempDir = "s3n://spark-redshift-testing/temp/"
...
sqlContext.sql("select * from diamonds")
  .write
...
diamonds_from_redshift.registerTempTable("tmp_diamonds")
...
display(diamonds_from_redshift.select("carat", "cut", "price").groupBy("carat", "cut").avg("price"))
...
val fromRedshiftQuery = sqlContext.read
  ...
  .option("url", jdbcUrl)
  .option("tempdir", tempDir)
  .option("query", "SELECT carat, cut, AVG(price) as avg_price from diamonds group by carat, cut")
  .load()
